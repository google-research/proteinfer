"""Tests for module parenthood_lib.py."""
...
class ParenthoodLibTest(parameterized.TestCase):
  ...
  def test_go_term_parsing_integration_test(self):
    gene_ontology_reference_full_file_path = 'testdata/go.obo'
    with open(gene_ontology_reference_full_file_path, 'r') as f:
  ...
  def test_parse_full_ec_file_to_transitive_parenthood_integration_test(self):
    path_to_dat_file = 'testdata/enzyme.dat'
    path_to_class_file = 'testdata/enzclass.txt'

    # Load input data.
    enzyme_commission_reference_dat_full_file_path = os.path.join(
        absltest.get_default_test_srcdir(), path_to_dat_file)
    with open(enzyme_commission_reference_dat_full_file_path) as f:
      whole_ec_dat_contents = f.read()

    enzyme_commission_reference_dat_full_file_path = os.path.join(
        absltest.get_default_test_srcdir(), path_to_class_file)
    with open(enzyme_commission_reference_dat_full_file_path) as f:
